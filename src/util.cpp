...
int nAnonymizeNoderxAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Noderx
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Noderx
    // Mac: ~/Library/Application Support/Noderx
    // Unix: ~/.Noderx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Noderx";
#else
    fs::path pathRet;
...
void createConf()       //Automatic Noderx.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Noderx.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "noderxd.pid"));
