...
{
    // Convert Noderx:// to Noderx:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Noderx://"))
    {
        uri.replace(0, 12, "Noderx:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Noderx.lnk";
}
...
{
    return GetAutostartDir() / "Noderx.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Noderx-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Noderx-qt [" + tr("command-line options") + "]                     " + "\n";
