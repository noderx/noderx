##################################  Notes  ##################################
# build:
#   docker build --no-cache -t noderx .
#
# run:
#   docker run -it -p 15015:15015 noderx
#
# run with a mounted directory for ~/.Noderx:
#   docker run -it -p 15015:15015 -v /path/to/a/local/directory:/root/.Noderx noderx
#
# run will exec you into docker /bin/bash
# from there, you can run:
# noderxd # starts the noderx deamon
#
# For accessing the noderx JSON-RPC api from the host:
# 1. Expose RPC port in when running docker
#    docker run -it -p 15015:15015 -p 5000:XXX -v /path/to/a/local/directory:/root/.Noderx noderx # Replace XXX with set rpcPort in noderx.conf
...
RUN git clone https://github.com/Noderx/Noderx.git Noderx && cd Noderx/src/ && make -f makefile.unix && cp noderxd /usr/local/sbin/noderxd

RUN mkdir /root/.Noderx/
